...
const Feeds = () => {
  ...
  useEffect(() => {
    const handleScroll = () => {
      if (window.scrollY > 0) { // Show button if scrolled down 300px
        setIsScrolled(true);
      } else {
        setIsScrolled(false);
      }
          ...
          <div className="w-3/4 p-6 bg-[#F1F0E8]">
            ...
            <div className="mt-8 ml-8 flex flex-col gap-6">
              {posts.map((post) => (
                <div
                  key={post.id}
                  className="w-full max-w-3xl p-6 bg-white rounded-lg shadow-lg hover:shadow-xl transition-transform transform hover:scale-105 relative"
                >
